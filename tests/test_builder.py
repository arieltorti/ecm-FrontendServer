...
def test_sim_basic(simulation_schema):
    ...
    assert any(
        lambda t: expectedt[t] == approx(result.timeline[t], 0.001) for t in range(10)
    )
...
def test_sim_missing_compartment(simulation_schema):
    ...
    simSIR = {
        "step": 5,
        "days": 50,
        "initial_conditions": {"S": 999600, "I": 400},
        "params": {"beta": 1, "gamma": 0.0714},
    }
    simulation = Simulation(**simSIR)
    sim = Simulator(ModelContext(model))
...
def test_sim_precondition_error_animate(simulation_schema):
    ...
    simSIR = {
        ...
        "days": 50,
        "initial_conditions": {"S": 999600, "I": 400, "R": 0},
        "params": {"beta": 0, "gamma": 0.0714},
        "iterate": {"key": "beta", "intervals": 10, "start": 10, "end": -10},
    }
    simulation = Simulation(**simSIR)
    sim = Simulator(ModelContext(model))
    with raises(SimulatorError) as e:
